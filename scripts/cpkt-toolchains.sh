...
print_darwin_target() {
  local candidate source root prefix strip
  ...
  printf 'status=ready\nroot=%s\nprefix=%s\ncc=%s\ncxx=%s\nld=%s\nar=%s\nranlib=%s\nnm=%s\notool=%s\n' \
    "$root" "$prefix" "$root/bin/$prefix-clang" "$root/bin/$prefix-clang++" "$root/bin/$prefix-ld" "$root/bin/$prefix-ar" "$root/bin/$prefix-ranlib" "$root/bin/$prefix-nm" "$root/bin/$prefix-otool"
  strip="$root/bin/$prefix-strip"
  if [[ -x "$strip" ]]; then
    printf 'strip=%s\n' "$strip"
  fi
}
