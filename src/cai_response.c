...
static int cai_json_builder_field_spooled(cai_json_builder *builder,
  ...
  if (lonejson_spooled_size(value) == 0U) {
    return CAI_OK;
  }
  if (*need_comma) {
...
static int cai_serialize_content_array_json(cai_json_builder *builder,
                                            const char *name,
                                            const lonejson_object_array *array,
                                            int emit_empty, int *need_comma,
                                            cai_error *error) {
  cai_response_content_doc *items;
  size_t i;
  int item_comma;
  int rc;

  if (array->count == 0U && !emit_empty) {
    return CAI_OK;
  }
  if (*need_comma) {
    rc = cai_json_builder_lit(builder, ",", error);
  ...
  items = (cai_response_content_doc *)array->items;
  for (i = 0U; rc == CAI_OK && i < array->count; i++) {
    ...
    item_comma = 0;
    if (rc == CAI_OK && items[i].type != NULL) {
      rc = cai_json_builder_field_string(builder, "type", items[i].type,
                                         &item_comma, error);
    }
    if (rc == CAI_OK) {
      rc = cai_json_builder_field_spooled(builder, "text", &items[i].text,
                                          &item_comma, error);
    }
    if (rc == CAI_OK) {
      rc = cai_json_builder_field_spooled(builder, "refusal",
                                          &items[i].refusal, &item_comma,
                                          error);
    }
    if (rc == CAI_OK) {
      rc = cai_json_builder_lit(builder, "}", error);
...
static int cai_serialize_history_output_json(cai_json_builder *builder,
                                             const cai_response_output_doc *item,
                                             cai_error *error) {
  int need_comma;
  int rc;
  ...
  need_comma = 0;
  rc = cai_json_builder_lit(builder, "{", error);
  if (rc == CAI_OK && item->type != NULL) {
    rc = cai_json_builder_field_string(builder, "type", item->type,
                                       &need_comma, error);
  }
  if (rc == CAI_OK && item->id != NULL) {
    rc = cai_json_builder_field_string(builder, "id", item->id, &need_comma,
                                       error);
  }
  if (rc == CAI_OK && item->type != NULL &&
      strcmp(item->type, "message") == 0 && item->role != NULL) {
    rc = cai_json_builder_field_string(builder, "role", item->role,
                                       &need_comma, error);
  }
  if (rc == CAI_OK && item->call_id != NULL) {
    rc = cai_json_builder_field_string(builder, "call_id", item->call_id,
                                       &need_comma, error);
  }
  if (rc == CAI_OK && item->name != NULL) {
    rc = cai_json_builder_field_string(builder, "name", item->name,
                                       &need_comma, error);
  }
  if (rc == CAI_OK && item->arguments != NULL) {
    rc = cai_json_builder_field_string(builder, "arguments", item->arguments,
                                       &need_comma, error);
  }
  if (rc == CAI_OK && item->created_by != NULL) {
    rc = cai_json_builder_field_string(builder, "created_by", item->created_by,
                                       &need_comma, error);
  }
  if (rc == CAI_OK) {
    rc = cai_json_builder_field_spooled(builder, "encrypted_content",
                                        &item->encrypted_content, &need_comma,
                                        error);
  }
  if (rc == CAI_OK) {
    rc = cai_serialize_content_array_json(
        builder, "summary", &item->summary,
        item->type != NULL && strcmp(item->type, "reasoning") == 0,
        &need_comma, error);
  }
  if (rc == CAI_OK) {
    rc = cai_serialize_content_array_json(builder, "content", &item->content, 0,
                                          &need_comma, error);
  }
  if (rc == CAI_OK) {
    rc = cai_json_builder_lit(builder, "}", error);
...
static int cai_capture_response_output_json(cai_response_doc *response_doc,
  ...
  for (i = 0U; rc == CAI_OK && i < response_doc->output.count; i++) {
    if (need_comma) {
      rc = cai_json_builder_lit(&builder, ",", error);
    }
    if (rc == CAI_OK) {
      rc = cai_serialize_history_output_json(&builder, &items[i], error);
    }
    if (rc == CAI_OK) {
      need_comma = 1;
    }
...
cai_serialize_content_parts_json(cai_json_builder *builder,
  ...
  for (j = 0U; rc == CAI_OK && j < content->count; j++) {
    ...
    if (rc == CAI_OK && parts[j].filename != NULL) {
      rc = cai_json_builder_field_string(builder, "filename", parts[j].filename,
                                         &part_comma, error);
    }
    ...
    if (rc == CAI_OK && parts[j].file_url != NULL) {
      rc = cai_json_builder_field_string(builder, "file_url", parts[j].file_url,
                                         &part_comma, error);
    }
  ...
  for (i = 0U; rc == CAI_OK && i < input->count; i++) {
    ...
    if (messages[i].kind == CAI_INPUT_FUNCTION_CALL_OUTPUT) {
      ...
      if (rc == CAI_OK && messages[i].content.count > 0U) {
        ...
        rc = cai_json_builder_string_spooled(
            builder, &messages[i].output_spooled, error);
      } else if (rc == CAI_OK) {
...
static int cai_response_create_params_build_input_json(
  ...
  if (rc == CAI_OK && has_raw_input && params->input.count > 0U) {
    rc = cai_json_builder_lit(&builder, ",", error);
  }
...
int cai_response_create_params_write_json_sink(
  ...
  memset(&context_item, 0, sizeof(context_item));
  memset(&input_json, 0, sizeof(input_json));
  memset(&tools_json, 0, sizeof(tools_json));
  memset(&input_reader, 0, sizeof(input_reader));
  memset(&tools_reader, 0, sizeof(tools_reader));
  has_input_json = 0;
  has_tools_json = 0;
  rc = CAI_OK;
  ...
  doc.conversation = params->conversation_id;
  doc.prompt_cache_key = params->prompt_cache_key;
  if (params->max_output_tokens > 0) {
    doc.max_output_tokens = params->max_output_tokens;
    doc.has_max_output_tokens = 1;
  }
  doc.reasoning.effort = params->reasoning_effort;
  doc.reasoning.summary = params->reasoning_summary;
  doc.text.format.type = params->text_format_type;
  doc.text.format.name = params->text_format_name;
  doc.text.format.description = params->text_format_description;
  lonejson_json_value_init(&doc.text.format.schema);
  if (params->text_format_schema_json != NULL) {
    lonejson_error_init(&json_error);
    if (lonejson_json_value_set_buffer(
            &doc.text.format.schema, params->text_format_schema_json,
    ...
    doc.text.format.strict = params->text_format_strict ? true : false;
    doc.text.format.has_strict = 1;
  }
  if (params->parallel_tool_calls >= 0) {
    doc.parallel_tool_calls = params->parallel_tool_calls ? true : false;
    doc.has_parallel_tool_calls = 1;
  }
  if (params->compact_threshold_tokens > 0LL) {
    context_item.type = "compaction";
    context_item.compact_threshold = params->compact_threshold_tokens;
    doc.context_management.items = &context_item;
    doc.context_management.count = 1U;
    doc.context_management.capacity = 1U;
    doc.context_management.elem_size = sizeof(context_item);
    doc.context_management.flags = LONEJSON_ARRAY_FIXED_CAPACITY;
  }
  lonejson_json_value_init(&doc.input);
  rc = cai_response_create_params_build_input_json(params, &input_json,
                                                   &input_reader, error);
  if (rc != CAI_OK) {
    goto done;
  }
  has_input_json = 1;
  lonejson_error_init(&json_error);
  if (lonejson_json_value_set_reader(&doc.input,
  ...
  lonejson_json_value_init(&doc.tools);
  if (params->tools.count > 0U) {
    rc = cai_response_create_params_build_tools_json(params, &tools_json,
                                                     &tools_reader, error);
    if (rc != CAI_OK) {
      goto done;
    }
    has_tools_json = 1;
    lonejson_error_init(&json_error);
    if (lonejson_json_value_set_reader(&doc.tools,
                                       cai_response_spooled_reader,
                                       &tools_reader,
                                       &json_error) != LONEJSON_STATUS_OK) {
      rc = cai_set_error_detail(error, CAI_ERR_TRANSPORT,
  ...
  lonejson_error_init(&json_error);
  rc = lonejson_serialize_sink(&cai_response_request_map, &doc,
  ...
  lonejson_json_value_cleanup(&doc.text.format.schema);
  lonejson_json_value_cleanup(&doc.input);
  lonejson_json_value_cleanup(&doc.tools);
  if (has_input_json) {
    lonejson_spooled_cleanup(&input_json);
  }
  if (has_tools_json) {
    lonejson_spooled_cleanup(&tools_json);
  }
  return rc;
}
