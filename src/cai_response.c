...
struct cai_response_request_upload {
  ...
  int curl_started;
#ifdef CAI_TESTING
  int test_tracked_open;
#endif
};

#ifdef CAI_TESTING
static int cai_test_response_request_upload_active_count = 0;

int cai_test_response_request_upload_active(void) {
  return cai_test_response_request_upload_active_count;
}
#endif
...
static const lonejson_field cai_response_request_fields[] = {
    LONEJSON_FIELD_STRING_ALLOC_OMIT_NULL(cai_response_request_doc, event_type,
                                          "type"),
...
static int cai_response_request_state_prepare(
    ...
    int omit_max_output_tokens, const char *event_type, cai_error *error) {
  lonejson_error json_error;
  int rc;
  ...
  state->doc.event_type = event_type;
...
int cai_response_create_params_write_json_sink(
  ...
  rc = cai_response_request_state_prepare(&state, params, stream, 0, 0, 0, NULL,
                                          error);
  if (rc != CAI_OK) {
  ...
  *out = NULL;
  ...
  lonejson_error_init(&json_error);
  {
    ...
    rc = cai_response_request_state_prepare(
        &count_state, params, stream, default_has_store, default_store,
        omit_max_output_tokens, event_type, error);
    if (rc == CAI_OK) {
  ...
  rc = cai_response_request_state_prepare(
      &upload->state, params, stream, default_has_store, default_store,
      omit_max_output_tokens, event_type, error);
  if (rc != CAI_OK) {
...
#ifdef CAI_TESTING
  upload->test_tracked_open = 1;
  cai_test_response_request_upload_active_count++;
#endif
  *out = upload;
  return CAI_OK;
}
...
#ifdef CAI_TESTING
  if (upload->test_tracked_open) {
    upload->test_tracked_open = 0;
    cai_test_response_request_upload_active_count--;
  }
#endif
