...
static lonejson_status cai_mcp_lonejson_http_request(
  ...
  write_context.max_response_bytes = request->max_response_bytes;
...
static int cai_mcp_oauth2_setup(cai_mcp_streamable_http_client_impl *impl,
  ...
  lonejson_error_init(&json_error);
  impl->auth_runtime = lonejson_new(NULL, &json_error);
...
static int cai_mcp_oauth2_fetch_client_credentials(
  ...
  if (http_response.status_code < 200L || http_response.status_code >= 300L) {
    ...
    lonejson_http_response_cleanup(&http_response);
    lonejson_owned_buffer_free(&body);
    return cai_set_error_http(error, CAI_ERR_SERVER, http_response.status_code,
...
static int cai_mcp_streaming_sse_buffer_pending_bytes(
    cai_mcp_streaming_response_context *context, const char *data, size_t len) {
  size_t i;

  for (i = 0U; i < len; i++) {
    context->rc = cai_mcp_streaming_sse_buffer_pending(context, data[i]);
    if (context->rc != 0) {
      return context->rc;
    }
  }
...
cai_mcp_streaming_sse_line(cai_mcp_streaming_response_context *context,
  ...
  if (strcmp(context->event, "message") != 0) {
    context->rc =
        cai_mcp_streaming_sse_buffer_pending_bytes(context, value, len);
    return context->rc;
  }
  if (context->data_started) {
    context->rc = cai_mcp_write_all_fd(context->write_fd, "\n", 1U);
...
static void *cai_mcp_streaming_post_thread(void *user) {
  ...
  if (rc == CAI_OK) {
    rc = cai_mcp_append_auth_header(context->impl, &headers, NULL);
  }
...
static int cai_mcp_sse_event_id_is_header_safe(const char *value, size_t len) {
  size_t i;
...
static int cai_mcp_sse_set_last_event_id(char **last_event_id,
  ...
  if (!cai_mcp_sse_event_id_is_header_safe(value, len)) {
    return cai_set_error(error, CAI_ERR_PROTOCOL,
                         "MCP SSE event id must be HTTP header safe");
  }
...
static long
cai_mcp_sse_retry_sleep_ms(cai_mcp_streamable_http_client_impl *impl,
                           long retry_ms) {
  long cap_ms;
  ...
  cap_ms = 1000L;
  if (impl != NULL && impl->timeout_ms > 0L && impl->timeout_ms < cap_ms) {
    cap_ms = impl->timeout_ms;
  }
  return retry_ms < cap_ms ? retry_ms : cap_ms;
}
...
cai_mcp_sse_normalize_response(cai_mcp_streamable_http_client_impl *impl,
  ...
  if (rc == CAI_OK && !final_seen && allow_resume &&
    ...
    if (resume.has_retry) {
      cai_mcp_sleep_ms(cai_mcp_sse_retry_sleep_ms(impl, resume.retry_ms));
    }
...
static int cai_mcp_parse_streamed_result_response(
  ...
  if (rc <= 0 || ch != '{') {
    return cai_set_error(error, CAI_ERR_PROTOCOL,
                         rc == 0 ? "MCP streamed response was missing "
                                   "JSON-RPC response"
                                 : "failed to parse MCP JSON-RPC response");
  }
  for (;;) {
...
static int cai_mcp_stream_request_result_once(
  ...
  while (rc != CAI_OK && !result_seen && context.rc == CAI_OK &&
    ...
    cai_mcp_clear_error(error);
    if (has_retry) {
      cai_mcp_sleep_ms(cai_mcp_sse_retry_sleep_ms(impl, retry_ms));
    }
    cai_mcp_http_response_capture_cleanup(response);
    if (pipe(fds) != 0) {
