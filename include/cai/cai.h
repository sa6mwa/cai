/** @file cai/cai.h
 *  @brief Public C89 API for cai OpenAI Responses clients, sessions, streams,
...
/** @brief Forward declaration for lockd/lonejson sources. */
struct lc_source;
/** @brief Forward declaration for lockd/lonejson sinks. */
struct lc_sink;
/** @brief Forward declaration for lonejson typed maps. */
struct lonejson_map;
/** @brief Forward declaration for lonejson spooled values. */
struct lonejson_spooled;
/** @brief Forward declaration for pslog loggers. */
struct pslog_logger;

/** @brief OpenAI API client handle. */
typedef struct cai_client cai_client;
/** @brief Agent facade handle that owns config, tools, and an implicit session. */
typedef struct cai_agent cai_agent;
/** @brief Stateful Responses session handle. */
typedef struct cai_session cai_session;
/** @brief Conversation handle wrapping a server-side conversation id. */
typedef struct cai_conversation cai_conversation;
/** @brief Streaming byte input handle. */
typedef struct cai_source cai_source;
/** @brief Streaming byte output handle. */
typedef struct cai_sink cai_sink;
/** @brief High-level response output wrapper. */
typedef struct cai_output cai_output;
/** @brief Low-level Responses create parameter builder. */
typedef struct cai_response_create_params cai_response_create_params;
/** @brief Low-level Responses API response handle. */
typedef struct cai_response cai_response;
/** @brief List response for response/conversation input items. */
typedef struct cai_input_item_list cai_input_item_list;
/** @brief Retrieved conversation item handle. */
typedef struct cai_conversation_item cai_conversation_item;
/** @brief Conversation items create parameter builder. */
typedef struct cai_conversation_items_params cai_conversation_items_params;
/** @brief Registry of callable local cai tools. */
typedef struct cai_tool_registry cai_tool_registry;
/** @brief Builder for JSON schema used by function tools. */
typedef struct cai_tool_schema cai_tool_schema;

/** @brief cai status codes returned by public APIs. */
typedef enum cai_status {
  /** @brief Operation completed successfully. */
  CAI_OK = 0,
  /** @brief Invalid argument, state, configuration, or input data. */
  CAI_ERR_INVALID = 1,
  /** @brief Allocation failed. */
  CAI_ERR_NOMEM = 2,
  /** @brief Transport layer failed. */
  CAI_ERR_TRANSPORT = 3,
  /** @brief Protocol parsing or validation failed. */
  CAI_ERR_PROTOCOL = 4,
  /** @brief Server returned an error response. */
  CAI_ERR_SERVER = 5,
  /** @brief Operation was cancelled. */
  CAI_ERR_CANCELLED = 6
} cai_status;

/** @brief Detailed error information populated by failing cai APIs. */
typedef struct cai_error {
  /** @brief cai_status code for the failure. */
  int code;
  /** @brief HTTP status from server responses, or zero when not applicable. */
  long http_status;
  /** @brief Human-readable error message owned by the cai_error. */
  char *message;
  /** @brief Additional diagnostic detail owned by the cai_error. */
  char *detail;
  /** @brief Server error code owned by the cai_error when provided. */
  char *server_code;
  /** @brief Server request id owned by the cai_error when provided. */
  char *request_id;
} cai_error;

/** @brief Custom allocation callback. */
typedef void *(*cai_malloc_fn)(void *context, size_t size);
/** @brief Custom reallocation callback. */
typedef void *(*cai_realloc_fn)(void *context, void *ptr, size_t size);
/** @brief Custom free callback. */
typedef void (*cai_free_fn)(void *context, void *ptr);

/** @brief Optional allocator callback table used by cai objects. */
typedef struct cai_allocator {
  /** @brief Allocation callback. */
  cai_malloc_fn malloc_fn;
  /** @brief Reallocation callback. */
  cai_realloc_fn realloc_fn;
  /** @brief Free callback. */
  cai_free_fn free_fn;
  /** @brief User context passed to allocator callbacks. */
  void *context;
} cai_allocator;

/** @brief Default OpenAI REST API base URL. */
#define CAI_OPENAI_BASE_URL "https://api.openai.com/v1"
/** @brief Default OpenRouter OpenAI-compatible REST API base URL. */
#define CAI_OPENROUTER_BASE_URL "https://openrouter.ai/api/v1"
/** @brief Default dotenv path used by explicit dotenv helper calls. */
#define CAI_DEFAULT_DOTENV_PATH ".env"
/** @brief Environment variable name for OpenAI API keys. */
#define CAI_OPENAI_API_KEY_ENV "OPENAI_API_KEY"
/** @brief Environment variable name for OpenRouter API keys. */
#define CAI_OPENROUTER_API_KEY_ENV "OPENROUTER_API_KEY"

/** @brief Configuration for opening a cai client. */
typedef struct cai_client_config {
  /** @brief API key string; when NULL cai looks at api_key_env. */
  const char *api_key;
  /** @brief Environment variable to read when api_key is NULL. */
  const char *api_key_env;
  /** @brief API base URL; NULL selects CAI_OPENAI_BASE_URL. */
  const char *base_url;
  /** @brief Optional OpenAI organization id header. */
  const char *organization_id;
  /** @brief Optional OpenAI project id header. */
  const char *project_id;
  /** @brief HTTP timeout in milliseconds; zero uses default. */
  long timeout_ms;
  /** @brief Non-zero disables HTTP/2 negotiation. */
  int http_2_disabled;
  /** @brief Non-zero disables TLS certificate verification. */
  int insecure_skip_verify;
  /** @brief Maximum non-streamed JSON response bytes; zero uses default. */
  size_t json_response_limit_bytes;
  /** @brief Optional pslog logger. */
  struct pslog_logger *logger;
  /** @brief Non-zero disables cai logging even when logger is set. */
  int logger_disabled;
  /** @brief Optional custom allocator callbacks. */
  cai_allocator allocator;
} cai_client_config;

/** @brief Use previous_response_id/server-side state for session continuity. */
#define CAI_SESSION_CONTINUITY_SERVER 0
/** @brief Use local client history when creating follow-up requests. */
#define CAI_SESSION_CONTINUITY_CLIENT_HISTORY 1
/** @brief Auto-select continuity mode based on provider behavior. */
#define CAI_SESSION_CONTINUITY_AUTO 2

/** @brief Configuration for constructing an agent facade. */
typedef struct cai_agent_config {
  /** @brief Model id for Responses requests. */
  const char *model;
  /** @brief Developer instructions sent as Responses instructions. */
  const char *developer_instructions;
  /** @brief Optional prompt cache key. */
  const char *prompt_cache_key;
  /** @brief Tool choice string such as CAI_TOOL_CHOICE_AUTO. */
  const char *tool_choice;
  /** @brief Raw JSON tool_choice override for provider-specific values. */
  const char *tool_choice_json;
  /** @brief Reasoning effort string such as CAI_REASONING_EFFORT_LOW. */
  const char *reasoning_effort;
  /** @brief Reasoning summary mode such as CAI_REASONING_SUMMARY_AUTO. */
  const char *reasoning_summary;
  /** @brief Structured output schema name. */
  const char *text_format_name;
  /** @brief Structured output schema description. */
  const char *text_format_description;
  /** @brief Structured output JSON schema. */
  const char *text_format_schema_json;
  /** @brief Non-zero requests strict structured output validation. */
  int text_format_strict;
  /** @brief Maximum output tokens; zero leaves unset. */
  int max_output_tokens;
  /** @brief Maximum tool calls; zero leaves unset. */
  int max_tool_calls;
  /** @brief Non-zero disables parallel tool calls. */
  int disable_parallel_tool_calls;
  /** @brief CAI_SESSION_CONTINUITY_* mode. */
  int session_continuity;
  /** @brief Non-zero disables default server-side auto-compaction. */
  int disable_auto_compaction;
  /** @brief Explicit server-side compaction token threshold; zero uses default. */
  long long compact_threshold_tokens;
  /** @brief Percent of known context window for compaction; zero uses default. */
  unsigned int compact_threshold_percent;
  /** @brief Non-zero records local history for export/import or client mode. */
  int enable_local_history;
  /** @brief Local history memory limit before spilling; zero uses default. */
  size_t history_memory_limit;
  /** @brief Optional local history spool directory. */
  const char *history_spool_dir;
} cai_agent_config;

/** @brief Low text verbosity setting for supported models. */
#define CAI_TEXT_VERBOSITY_LOW "low"
/** @brief Medium text verbosity setting for supported models. */
#define CAI_TEXT_VERBOSITY_MEDIUM "medium"
/** @brief High text verbosity setting for supported models. */
#define CAI_TEXT_VERBOSITY_HIGH "high"

/** @brief Responses truncation mode that lets the API decide. */
#define CAI_RESPONSE_TRUNCATION_AUTO "auto"
/** @brief Responses truncation mode that disables truncation. */
#define CAI_RESPONSE_TRUNCATION_DISABLED "disabled"

/** @brief Service tier that lets the API select the tier. */
#define CAI_SERVICE_TIER_AUTO "auto"
/** @brief Default service tier. */
#define CAI_SERVICE_TIER_DEFAULT "default"
/** @brief Flex service tier. */
#define CAI_SERVICE_TIER_FLEX "flex"
/** @brief Priority service tier. */
#define CAI_SERVICE_TIER_PRIORITY "priority"

/** @brief Hosted OpenAI web search tool type. */
#define CAI_HOSTED_TOOL_WEB_SEARCH "web_search"
/** @brief Hosted OpenAI file search tool type. */
#define CAI_HOSTED_TOOL_FILE_SEARCH "file_search"
/** @brief Hosted OpenAI MCP connector tool type. */
#define CAI_HOSTED_TOOL_MCP "mcp"
/** @brief Hosted OpenAI computer-use preview tool type. */
#define CAI_HOSTED_TOOL_COMPUTER_USE "computer_use_preview"
/** @brief Hosted OpenAI image generation tool type. */
#define CAI_HOSTED_TOOL_IMAGE_GENERATION "image_generation"
/** @brief Hosted OpenAI code interpreter tool type. */
#define CAI_HOSTED_TOOL_CODE_INTERPRETER "code_interpreter"
/** @brief Hosted OpenAI tool search tool type. */
#define CAI_HOSTED_TOOL_TOOL_SEARCH "tool_search"

/** @brief Disable reasoning effort where supported. */
#define CAI_REASONING_EFFORT_NONE "none"
/** @brief Minimal reasoning effort. */
#define CAI_REASONING_EFFORT_MINIMAL "minimal"
/** @brief Low reasoning effort. */
#define CAI_REASONING_EFFORT_LOW "low"
/** @brief Medium reasoning effort. */
#define CAI_REASONING_EFFORT_MEDIUM "medium"
/** @brief High reasoning effort. */
#define CAI_REASONING_EFFORT_HIGH "high"
/** @brief Extra-high reasoning effort. */
#define CAI_REASONING_EFFORT_XHIGH "xhigh"

/** @brief Let the API decide whether to stream reasoning summaries. */
#define CAI_REASONING_SUMMARY_AUTO "auto"
/** @brief Concise reasoning summary mode. */
#define CAI_REASONING_SUMMARY_CONCISE "concise"
/** @brief Detailed reasoning summary mode. */
#define CAI_REASONING_SUMMARY_DETAILED "detailed"

/** @brief Automatic tool choice. */
#define CAI_TOOL_CHOICE_AUTO "auto"
/** @brief Disable tool calls. */
#define CAI_TOOL_CHOICE_NONE "none"
/** @brief Require a tool call. */
#define CAI_TOOL_CHOICE_REQUIRED "required"

/** @brief Configuration for hosted MCP tools handled by the Responses API. */
typedef struct cai_hosted_mcp_tool_config {
  /** @brief Server label exposed to the API. */
  const char *server_label;
  /** @brief Remote MCP server URL. */
  const char *server_url;
  /** @brief Optional hosted connector id. */
  const char *connector_id;
  /** @brief Optional server description. */
  const char *server_description;
  /** @brief Optional raw JSON object of headers for the hosted MCP server. */
  const char *headers_json;
  /** @brief Optional raw JSON allowed_tools value. */
  const char *allowed_tools_json;
  /** @brief Optional allow-list of hosted tool names. */
  const char *const *allowed_tool_names;
  /** @brief Number of entries in allowed_tool_names. */
  size_t allowed_tool_name_count;
  /** @brief Optional raw JSON require_approval policy. */
  const char *require_approval_json;
} cai_hosted_mcp_tool_config;

/** @brief Tool event emitted before a local tool runs. */
#define CAI_TOOL_EVENT_START 1
/** @brief Tool event emitted after local tool output is available. */
#define CAI_TOOL_EVENT_OUTPUT 2
/** @brief Tool event emitted when a local tool fails. */
#define CAI_TOOL_EVENT_ERROR 3

/** @brief Observable tool execution event for auto-run sessions and agents. */
typedef struct cai_tool_event {
  /** @brief CAI_TOOL_EVENT_* event type. */
  int type;
  /** @brief Tool name. */
  const char *name;
  /** @brief Tool arguments as JSON when available as a string. */
  const char *arguments_json;
  /** @brief Tool arguments as a spooled JSON value when large. */
  const struct lonejson_spooled *arguments_json_spooled;
  /** @brief Tool output as a spooled JSON value for output events. */
  const struct lonejson_spooled *output_json;
  /** @brief Tool error for error events. */
  const cai_error *tool_error;
} cai_tool_event;

/** @brief Callback invoked for tool start/output/error events. */
typedef int (*cai_tool_event_fn)(void *context, const cai_tool_event *event,
                                 cai_error *error);

/** @brief Common cursor pagination parameters. */
typedef struct cai_list_params {
  /** @brief Cursor id after which to list. */
  const char *after;
  /** @brief Maximum number of items to return; zero leaves unset. */
  int limit;
  /** @brief Sort order string accepted by the API. */
  const char *order;
} cai_list_params;

/** @brief Options controlling local tool auto-run loops. */
typedef struct cai_run_options {
  /** @brief Maximum auto-run tool rounds; zero uses default. */
  int max_tool_rounds;
  /** @brief Non-zero disables local tool auto-run. */
  int disable_tool_auto_run;
  /** @brief In-memory tool output spool limit before spilling. */
  size_t tool_output_memory_limit;
  /** @brief Maximum accepted tool output bytes; zero uses default. */
  size_t tool_output_max_bytes;
  /** @brief Optional directory for tool output spill files. */
  const char *tool_spool_dir;
  /** @brief Optional callback for observable tool events. */
  cai_tool_event_fn tool_event;
  /** @brief Context passed to tool_event. */
  void *tool_event_context;
} cai_run_options;

/** @brief Token usage counters reported by Responses. */
typedef struct cai_token_usage {
  /** @brief Input tokens charged for the request. */
  long long input_tokens;
  /** @brief Input tokens served from prompt cache. */
  long long input_cached_tokens;
  /** @brief Output tokens produced by the model. */
  long long output_tokens;
  /** @brief Output tokens attributed to reasoning. */
  long long output_reasoning_tokens;
  /** @brief Total tokens reported by the API. */
  long long total_tokens;
} cai_token_usage;

/** @brief Source read callback. Returns bytes read, 0 for EOF or error. */
typedef size_t (*cai_source_read_fn)(void *context, void *buffer, size_t count,
                                     cai_error *error);
/** @brief Source reset callback for seekable/replayable sources. */
typedef int (*cai_source_reset_fn)(void *context, cai_error *error);
/** @brief Source close callback. */
typedef void (*cai_source_close_fn)(void *context);

/** @brief Callback table for custom streaming input sources. */
typedef struct cai_source_callbacks {
  /** @brief Read bytes into buffer. */
  cai_source_read_fn read;
  /** @brief Reset source to the beginning, or NULL if unsupported. */
  cai_source_reset_fn reset;
  /** @brief Close callback invoked by cai_source_close. */
  cai_source_close_fn close;
  /** @brief User context passed to callbacks. */
  void *context;
} cai_source_callbacks;

/** @brief Sink write callback. */
...
/** @brief Callback table for custom streaming output sinks. */
typedef struct cai_sink_callbacks {
  /** @brief Write bytes from a streaming producer. */
  cai_sink_write_fn write;
  /** @brief Close callback invoked by cai_sink_close. */
  cai_sink_close_fn close;
  /** @brief User context passed to callbacks. */
  void *context;
} cai_sink_callbacks;

/** @brief Callback returning dynamic stream prefix/suffix text. */
typedef const char *(*cai_stream_affix_fn)(void *context);
/** @brief Callback for incremental function-call argument deltas. */
typedef int (*cai_stream_function_call_delta_fn)(void *context,
                                                 const char *item_id,
                                                 int output_index,
                                                 const char *delta,
                                                 cai_error *error);
/** @brief Callback for completed streamed function-call arguments. */
typedef int (*cai_stream_function_call_done_fn)(void *context,
                                                ...
                                                cai_error *error);
/** @brief Callback for completed streamed output items. */
typedef int (*cai_stream_output_item_done_fn)(void *context,
                                              ...
                                              cai_error *error);
/** @brief Callback for incremental output text deltas. */
...
/** @brief Static or dynamic prefix/suffix emitted around stream sections. */
typedef struct cai_stream_affix {
  /** @brief Static text to emit, or NULL. */
  const char *text;
  /** @brief Dynamic callback used when text is NULL. */
  cai_stream_affix_fn callback;
  /** @brief Context passed to callback. */
  void *context;
} cai_stream_affix;

/** @brief Stream routing table for reasoning summaries, text, and tool calls. */
typedef struct cai_stream_sinks {
  /** @brief Sink for reasoning summary deltas. */
  cai_sink *reasoning_summary;
  /** @brief Sink for final output text deltas. */
  cai_sink *output_text;
  /** @brief Prefix emitted before each reasoning summary segment. */
  cai_stream_affix reasoning_summary_prefix;
  /** @brief Suffix emitted after each reasoning summary segment. */
  cai_stream_affix reasoning_summary_suffix;
  /** @brief Prefix emitted before the first output text segment. */
  cai_stream_affix output_text_prefix;
  /** @brief Suffix emitted after output text completes. */
  cai_stream_affix output_text_suffix;
  /** @brief Callback for streamed function-call argument deltas. */
  cai_stream_function_call_delta_fn function_call_arguments_delta;
  /** @brief Callback for completed function-call arguments. */
  cai_stream_function_call_done_fn function_call_arguments_done;
  /** @brief Context passed to function-call callbacks. */
  void *function_call_context;
  /** @brief Callback for completed output items. */
  cai_stream_output_item_done_fn output_item_done;
  /** @brief Context passed to output_item_done. */
  void *output_item_context;
  /** @brief Callback for output text deltas in addition to output_text sink. */
  cai_stream_output_text_delta_fn output_text_delta;
  /** @brief Context passed to output_text_delta. */
  void *output_text_context;
} cai_stream_sinks;

/** @brief Typed lonejson-backed tool callback. */
typedef int (*cai_tool_fn)(void *context, const void *params, void *result,
                           cai_error *error);
/** @brief Alias for typed lonejson-backed tool callbacks. */
typedef cai_tool_fn cai_tool_lonejson_fn;
/** @brief Raw JSON tool callback for small argument strings. */
typedef int (*cai_tool_raw_fn)(void *context, const char *arguments_json,
                               cai_sink *output, cai_error *error);
/** @brief Spooled raw JSON tool callback for large argument values. */
typedef int (*cai_tool_raw_spooled_fn)(void *context,
                                       struct lonejson_spooled *arguments_json,
                                       cai_sink *output, cai_error *error);

/** @brief Duplicate a string using cai ownership for typed tool results. */
char *cai_tool_result_strdup(const char *value, cai_error *error);
/** @brief Set a typed tool result field from a file path source. */
...
/** @brief Set a typed tool result field from a spooled JSON/string value. */
int cai_tool_result_set_spooled(const struct lonejson_map *result_map,
                                void *result, const char *field_name,
                                struct lonejson_spooled *spool,
                                cai_error *error);

/** @brief Public method-table facade for a cai client. */
struct cai_client {
  /** @brief Create a new agent bound to this client. */
  int (*new_agent)(cai_client *client, const cai_agent_config *config,
                   cai_agent **out, cai_error *error);
  /** @brief Create a new server-side conversation. */
  int (*create_conversation)(cai_client *client, cai_conversation **out,
                             cai_error *error);
  /** @brief Close and destroy the client. */
  void (*close)(cai_client *client);
  /** @brief Private implementation pointer; do not access directly. */
  void *impl;
};

/** @brief Public method-table facade for a high-level agent. */
struct cai_agent {
  /** @brief Register a typed lonejson-backed local tool. */
  ...
  /** @brief Register a raw JSON local tool. */
  int (*register_raw_tool)(cai_agent *agent, const char *name,
                           ...
                           cai_error *error);
  /** @brief Register a spooled raw JSON local tool. */
...
struct cai_session {
  ...
  /** @brief Open output text as a streaming source. */
  int (*open_text_source)(cai_session *session, cai_source **out,
                          cai_error *error);
  /** @brief Add one user text message and run the session. */
  int (*send_text)(cai_session *session, const char *text, cai_response **out,
                   cai_error *error);
  /** @brief Return last recorded usage for the session. */
  int (*last_usage)(const cai_session *session, cai_token_usage *out,
                    cai_error *error);
  /** @brief Return the model context window in tokens, or zero if unknown. */
  long long (*context_window_tokens)(const cai_session *session);
  /** @brief Return the configured auto-compaction token threshold. */
  long long (*auto_compact_token_limit)(const cai_session *session);
  /** @brief Return estimated context-window usage percentage. */
  int (*context_percent)(const cai_session *session, double *out,
                         cai_error *error);
  /** @brief Return non-zero if local history has spilled to disk. */
  int (*history_spilled)(const cai_session *session);
  /** @brief Export local history as a streaming source. */
  int (*export_history_source)(cai_session *session, cai_source **out,
                               cai_error *error);
  /** @brief Import local history from a streaming source. */
  int (*import_history_source)(cai_session *session, cai_source *source,
                               cai_error *error);
  /** @brief Export complete session state as a streaming source. */
  int (*export_state_source)(cai_session *session, cai_source **out,
                             cai_error *error);
  /** @brief Import complete session state from a streaming source. */
  int (*import_state_source)(cai_session *session, cai_source *source,
                             cai_error *error);
  /** @brief Save complete session state to a filesystem path. */
  int (*save_state_path)(cai_session *session, const char *path,
                         cai_error *error);
  /** @brief Load complete session state from a filesystem path. */
  int (*load_state_path)(cai_session *session, const char *path,
                         cai_error *error);
  /** @brief Close and destroy the session. */
  void (*close)(cai_session *session);
  /** @brief Private implementation pointer; do not access directly. */
  void *impl;
};

/** @brief Public method-table facade for function tool schema building. */
struct cai_tool_schema {
  /** @brief Enable or disable strict tool schema generation. */
  ...
  int (*boolean)(cai_tool_schema *schema, const char *name,
                 const char *description, int required, cai_error *error);
  /** @brief Add a string enum property. */
...
/** @brief Register a typed lonejson-backed tool on an agent. */
...
int cai_agent_new_conversation_session(cai_agent *agent, cai_session **out,
                                       cai_error *error);
/** @brief Create a session bound to an existing conversation. */
...
/** @brief Bind a session to a server-side conversation id. */
int cai_session_set_conversation_id(cai_session *session,
                                    const char *conversation_id,
                                    cai_error *error);
/** @brief Bind a session to a conversation handle. */
int cai_session_set_conversation(cai_session *session,
                                 const cai_conversation *conversation,
                                 cai_error *error);
/** @brief Return the session conversation id, or NULL. */
...
int cai_session_run_output(cai_session *session, cai_output **out,
                           cai_error *error);
/** @brief Initialize local tool auto-run options. */
...
int cai_session_open_text_source(cai_session *session, cai_source **out,
                                 cai_error *error);
/** @brief Run experimental client-side manual compaction. */
...
int cai_session_send_text(cai_session *session, const char *text,
                          cai_response **out, cai_error *error);
/** @brief Return last recorded token usage for the session. */
...
/** @brief Create a sink that writes to stderr. */
...
int cai_response_create_params_add_hosted_mcp_tool(
    ...
    cai_error *error);
/** @brief Add raw JSON function-call output. */
int cai_response_create_params_add_function_call_output(
    ...
    cai_error *error);
/** @brief Add text function-call output. */
int cai_response_create_params_add_function_call_output_text(
    ...
    cai_error *error);
/** @brief Add image URL function-call output. */
int cai_response_create_params_add_function_call_output_image_url(
    ...
    const char *detail, cai_error *error);
/** @brief Add file id function-call output. */
int cai_response_create_params_add_function_call_output_file_id(
    ...
    const char *detail, cai_error *error);
/** @brief Add spooled file data function-call output. */
...
/** @brief Write response output text to a sink. */
int cai_response_write_output_text(const cai_response *response, cai_sink *sink,
                                   cai_error *error);
/** @brief Write response refusal text to a sink. */
