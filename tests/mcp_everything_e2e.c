...
int main(int argc, char **argv) {
  ...
  cai_error_cleanup(&error);
  cai_error_init(&error);
  if (e2e_spool(&context_arguments, "[]") != 0) {
    cai_sink_close(sink);
    cai_mcp_client_destroy(client);
    free(writer.data);
    return 1;
  }
  rc = cai_mcp_client_call_tool(client, "echo", &context_arguments, sink,
                                &error);
  context_arguments.cleanup(&context_arguments);
  ...
  rc = cai_mcp_client_complete(client, "ref/prompt", "completable-prompt",
                               "name", "", &context_arguments, sink, &error);
  context_arguments.cleanup(&context_arguments);
  ...
  if (e2e_expect_error(
          "Everything unknown method", rc, &error, CAI_ERR_PROTOCOL,
          "MCP streamed response was missing JSON-RPC response") != 0) {
    cai_sink_close(sink);
    cai_mcp_client_destroy(client);
    free(writer.data);
    cai_error_cleanup(&error);
  ...
  if (e2e_expect_error(
          "Everything missing resource", rc, &error, CAI_ERR_PROTOCOL,
          "MCP streamed response was missing JSON-RPC response") != 0) {
    cai_sink_close(sink);
    cai_mcp_client_destroy(client);
    free(writer.data);
  ...
  if (e2e_expect_error(
          "Everything missing prompt", rc, &error, CAI_ERR_PROTOCOL,
          "MCP streamed response was missing JSON-RPC response") != 0) {
    cai_sink_close(sink);
    cai_mcp_client_destroy(client);
    free(writer.data);
