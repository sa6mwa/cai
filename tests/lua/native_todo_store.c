...
typedef struct native_blob_store_state {
  const char *value;
  size_t offset;
  int load_count;
  int replace_count;
} native_blob_store_state;

static native_todo_store_state native_todo_store;
static native_session_store_state native_session_store;
static native_blob_store_state native_blob_store;
...
static size_t native_blob_store_read(void *context, void *buffer, size_t count,
                                     cai_error *error) {
  native_blob_store_state *store;
  size_t remaining;
  size_t amount;

  (void)error;
  store = (native_blob_store_state *)context;
  if (store == NULL || store->value == NULL ||
      store->offset >= strlen(store->value)) {
    return 0U;
  }
  remaining = strlen(store->value) - store->offset;
  amount = remaining < count ? remaining : count;
  memcpy(buffer, store->value + store->offset, amount);
  store->offset += amount;
  return amount;
}

static int native_blob_store_load(void *context, const char *key,
  ...
  *out = NULL;
  ...
  callbacks.read = native_blob_store_read;
  callbacks.context = store;
  return cai_source_from_callbacks(&callbacks, out, error);
}

static int native_blob_store_replace(void *context, const char *key,
  ...
  while (cai_source_read(value, buffer, sizeof(buffer), error) != 0U) {
  }
...
static cai_blob_store native_blob_store_callbacks = {
    native_blob_store_load, native_blob_store_replace, &native_blob_store};
...
static int native_blob_store_new(lua_State *L) {
  return native_store_new(L, "blob", &native_blob_store_callbacks, NULL, 0);
}
...
static int native_blob_store_reset(lua_State *L) {
  (void)L;
  memset(&native_blob_store, 0, sizeof(native_blob_store));
  native_blob_store.value =
      "{\"auth_mode\":\"chatgpt\",\"tokens\":{\"access_token\":"
      "\"eyJhbGciOiJub25lIn0.eyJleHAiOjQxMDI0NDQ4MDB9.native\","
      "\"refresh_token\":\"native-refresh\",\"account_id\":"
      "\"native-account\"},\"last_refresh\":\"2026-01-01T00:00:00Z\"}";
  return 0;
}
