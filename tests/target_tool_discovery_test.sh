...
verify_script=$repo_root/scripts/verify_release_artifacts.sh
if grep -F 'command -v readelf' "$verify_script" >/dev/null; then
  printf 'release package verification must not use ambient readelf discovery\n' >&2
  exit 1
fi
...
if ! grep -F 'dynamic=$("$readelf" -d "$so"' "$verify_script" >/dev/null; then
  printf 'release package verification must run the discovered readelf\n' >&2
  exit 1
fi
