...
static int test_mcp_client_set_log_level(cai_mcp_client *client,
                                         const char *level, cai_error *error) {
  test_mcp_client_impl *impl;
  ...
  impl->set_log_level_count++;
  snprintf(impl->last_log_level, sizeof(impl->last_log_level), "%s",
           level != NULL ? level : "");
  return CAI_OK;
}
...
static void test_mcp_client_receiver_surface(test_state *state) {
  ...
  cai_sink_close(sink);
  expect_int(
      state, "mcp_client_receiver_set_log_level",
      cai_mcp_client_set_log_level(&fake.public_client, "warning", &error),
      CAI_OK);
  expect_int(state, "mcp_client_receiver_set_log_level_count",
...
static void mock_no_request_child(int pipe_fd) {
  struct sockaddr_in addr;
  socklen_t addr_len;
  fd_set read_set;
  ...
  do {
    ready = select(server_fd + 1, &read_set, NULL, NULL, &timeout);
...
static int http_mock_server_open_no_request(test_state *state, const char *name,
                                            http_mock_server *server) {
  int pipe_fds[2];
  int port;
  ...
  if (server->pid == 0) {
    close(pipe_fds[0]);
    mock_no_request_child(pipe_fds[1]);
  }
  close(pipe_fds[1]);
  if (mock_read_port(pipe_fds[0], &port) != 0) {
    close(pipe_fds[0]);
...
typedef struct test_mcp_notification_state {
  int count;
  int fail;
  char method[128];
  char params[512];
} test_mcp_notification_state;

typedef struct test_mcp_roots_state {
  int count;
  int fail;
...
static int test_mcp_notification_callback(void *context, const char *method,
                                          lonejson_spooled *params_json,
                                          cai_error *error) {
  test_mcp_notification_state *state;
  char *params;

  state = (test_mcp_notification_state *)context;
  if (state == NULL || method == NULL) {
    return cai_set_error(error, CAI_ERR_INVALID,
                         "notification callback context is required");
  }
  state->count++;
  snprintf(state->method, sizeof(state->method), "%s", method);
  state->params[0] = '\0';
  if (params_json != NULL) {
    params = test_spooled_to_cstr(params_json);
    if (params == NULL) {
      return cai_set_error(error, CAI_ERR_TRANSPORT,
                           "failed to read notification params");
    }
...
static int test_mcp_roots_list_callback(void *context, cai_sink *result_json,
                                        cai_error *error) {
  test_mcp_roots_state *state;
  const char *json;

  state = (test_mcp_roots_state *)context;
  if (state == NULL || result_json == NULL) {
    return cai_set_error(error, CAI_ERR_INVALID,
                         "roots callback context is required");
  }
  state->count++;
  if (state->fail) {
...
static void test_mcp_streamable_http_client_roundtrip(test_state *state) {
  ...
  static const char *logging_set_level_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: session-123",
      "MCP-Protocol-Version: " CAI_MCP_PROTOCOL_VERSION,
  ...
  expect_str(state, "mcp_streamable_call_output", writer.buffer,
             ...
             "\"isError\":false}");
  expect_int(state, "mcp_streamable_notification_count", notifications.count,
             1L);
...
static void
test_mcp_streamable_http_response_jsonrpc_invalid(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
      "\"version\":\"1\"}}}";
  static const char ping_body[] =
      "{\"jsonrpc\":\"2.1\",\"id\":2,\"result\":{}}";
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\""};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: jsonrpc-invalid-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: jsonrpc-invalid-session",
      "\"id\":2", "\"method\":\"ping\""};
  ...
  memset(&server, 0, sizeof(server));
  cai_error_init(&error);
  if (http_mock_server_open_script(state, "mcp_streamable_jsonrpc_invalid_mock",
                                   script, sizeof(script) / sizeof(script[0]),
                                   &server) != 0) {
    cai_error_cleanup(&error);
    return;
  ...
  config.url = url;
  config.timeout_ms = 500L;
  expect_int(state, "mcp_streamable_jsonrpc_invalid_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_jsonrpc_invalid_ping",
             cai_mcp_client_ping(client, &error), CAI_ERR_PROTOCOL);
  expect_str(state, "mcp_streamable_jsonrpc_invalid_message", error.message,
             "MCP JSON-RPC version must be 2.0");
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_jsonrpc_invalid_mock", server.pid,
                    &server.child_status);
}

static void test_mcp_streamable_http_sse_jsonrpc_invalid(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
  ...
  static const char ping_body[] =
      "event: message\n"
      "data: {\"jsonrpc\":\"2.1\",\"method\":\"notifications/message\","
      "\"params\":{\"level\":\"info\",\"data\":\"bad version\"}}\n\n"
      "event: message\n"
      "data: {\"jsonrpc\":\"2.0\",\"id\":2,\"result\":{}}\n\n";
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\""};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: sse-jsonrpc-invalid-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: sse-jsonrpc-invalid-session",
      "\"id\":2", "\"method\":\"ping\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
       "application/json",
       "req-init\r\nMCP-Session-Id: sse-jsonrpc-invalid-session",
       initialize_body},
      {"POST /v1/mcp HTTP/", initialized_required,
       sizeof(initialized_required) / sizeof(initialized_required[0]), NULL, 0U,
       200, "OK", "application/json", NULL, "{}"},
  ...
  memset(&server, 0, sizeof(server));
  cai_error_init(&error);
  if (http_mock_server_open_script(state, "mcp_streamable_sse_jsonrpc_mock",
                                   script, sizeof(script) / sizeof(script[0]),
                                   &server) != 0) {
    cai_error_cleanup(&error);
    return;
  ...
  config.url = url;
  config.timeout_ms = 500L;
  expect_int(state, "mcp_streamable_sse_jsonrpc_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_sse_jsonrpc_ping",
             cai_mcp_client_ping(client, &error), CAI_ERR_PROTOCOL);
  expect_str(state, "mcp_streamable_sse_jsonrpc_message", error.message,
             "MCP JSON-RPC version must be 2.0");
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_sse_jsonrpc_mock", server.pid,
                    &server.child_status);
}

static void
test_mcp_streamable_http_response_missing_result_error(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
  ...
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\""};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: missing-result-error-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: missing-result-error-session",
      "\"id\":2", "\"method\":\"ping\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
       "application/json",
       "req-init\r\nMCP-Session-Id: missing-result-error-session",
       initialize_body},
      {"POST /v1/mcp HTTP/", initialized_required,
       sizeof(initialized_required) / sizeof(initialized_required[0]), NULL, 0U,
       200, "OK", "application/json", NULL, "{}"},
  ...
  memset(&server, 0, sizeof(server));
  cai_error_init(&error);
  if (http_mock_server_open_script(state, "mcp_streamable_missing_result_mock",
                                   script, sizeof(script) / sizeof(script[0]),
                                   &server) != 0) {
    cai_error_cleanup(&error);
    return;
  ...
  config.url = url;
  config.timeout_ms = 500L;
  expect_int(state, "mcp_streamable_missing_result_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_missing_result_ping",
             cai_mcp_client_ping(client, &error), CAI_ERR_PROTOCOL);
  expect_str(
      state, "mcp_streamable_missing_result_message", error.message,
      "MCP JSON-RPC response must include exactly one of result or error");
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_missing_result_mock", server.pid,
                    &server.child_status);
}

static void
test_mcp_streamable_http_response_result_and_error(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
      "\"version\":\"1\"}}}";
  static const char ping_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":2,\"result\":{},\"error\":{\"code\":"
      "-32000,\"message\":\"conflicting\"}}";
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\""};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: result-and-error-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: result-and-error-session",
      "\"id\":2", "\"method\":\"ping\""};
  ...
  memset(&server, 0, sizeof(server));
  cai_error_init(&error);
  if (http_mock_server_open_script(state, "mcp_streamable_result_error_mock",
                                   script, sizeof(script) / sizeof(script[0]),
                                   &server) != 0) {
    cai_error_cleanup(&error);
    return;
  ...
  config.url = url;
  config.timeout_ms = 500L;
  expect_int(state, "mcp_streamable_result_error_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_result_error_ping",
             cai_mcp_client_ping(client, &error), CAI_ERR_PROTOCOL);
  expect_str(
      state, "mcp_streamable_result_error_message", error.message,
      "MCP JSON-RPC response must include exactly one of result or error");
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_result_error_mock", server.pid,
                    &server.child_status);
}

static void test_mcp_streamable_http_response_null_result(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
  ...
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\""};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: null-result-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {"POST /v1/mcp HTTP/",
                                        "MCP-Session-Id: null-result-session",
                                        "\"id\":2", "\"method\":\"ping\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
  ...
  memset(&server, 0, sizeof(server));
  cai_error_init(&error);
  if (http_mock_server_open_script(state, "mcp_streamable_null_result_mock",
                                   script, sizeof(script) / sizeof(script[0]),
                                   &server) != 0) {
    cai_error_cleanup(&error);
    return;
  ...
  config.url = url;
  config.timeout_ms = 500L;
  expect_int(state, "mcp_streamable_null_result_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_null_result_ping",
             cai_mcp_client_ping(client, &error), CAI_OK);
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_null_result_mock", server.pid,
                    &server.child_status);
}
...
static void test_mcp_streamable_http_logging_error(test_state *state) {
  static const char initialize_body[] =
      ...
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
      "\"version\":\"1\"}}}";
  static const char logging_error_body[] =
  ...
  static const char *initialized_required[] = {
      ...
      "\"method\":\"notifications/initialized\""};
  static const char *logging_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: logging-session", "\"id\":2",
      "\"method\":\"logging/setLevel\"", "\"level\":\"debug\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
  ...
  memset(&server, 0, sizeof(server));
  cai_error_init(&error);
  if (http_mock_server_open_script(state, "mcp_streamable_logging_error_mock",
                                   script, sizeof(script) / sizeof(script[0]),
                                   &server) != 0) {
    cai_error_cleanup(&error);
    return;
  ...
  expect_int(state, "mcp_streamable_logging_error_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_logging_error_call",
             cai_mcp_client_set_log_level(client, "debug", &error),
             CAI_ERR_SERVER);
  expect_str(state, "mcp_streamable_logging_error_message", error.message,
             "invalid log level");
  cai_error_cleanup(&error);
  cai_error_init(&error);
  expect_int(state, "mcp_streamable_logging_invalid_level",
             cai_mcp_client_set_log_level(client, "verbose", &error),
             CAI_ERR_INVALID);
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
...
static void
test_mcp_streamable_http_notification_callback_error(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
  ...
  static const char ping_body[] =
      ...
      "\"level\":\"warning\",\"logger\":\"mock\",\"data\":\"heads up\"}}\n\n"
      "event: message\n"
      "data: {\"jsonrpc\":\"2.0\",\"id\":2,\"result\":{}}\n\n";
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\""};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: notification-error-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: notification-error-session",
      "\"id\":2", "\"method\":\"ping\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
       "application/json",
       "req-init\r\nMCP-Session-Id: notification-error-session",
       initialize_body},
      {"POST /v1/mcp HTTP/", initialized_required,
       sizeof(initialized_required) / sizeof(initialized_required[0]), NULL, 0U,
       200, "OK", "application/json", NULL, "{}"},
  ...
  memset(&server, 0, sizeof(server));
  memset(&notifications, 0, sizeof(notifications));
  notifications.fail = 1;
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_notification_callback_error_mock", script,
          sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  ...
  config.notification = test_mcp_notification_callback;
  config.notification_context = &notifications;
  expect_int(state, "mcp_streamable_notification_error_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_notification_error_call",
             cai_mcp_client_ping(client, &error), CAI_ERR_CANCELLED);
  expect_int(state, "mcp_streamable_notification_error_count",
             notifications.count, 1L);
  expect_str(state, "mcp_streamable_notification_error_method",
             notifications.method, "notifications/message");
  expect_substr(state, "mcp_streamable_notification_error_params",
                notifications.params, "\"level\":\"warning\"");
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_notification_callback_error_mock",
                    server.pid, &server.child_status);
}
...
static void test_mcp_streamable_http_roots_list_request(test_state *state) {
  ...
  static const char *init_required[] = {
      ...
      "\"capabilities\":{\"roots\":{\"listChanged\":true}}"};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: roots-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {"POST /v1/mcp HTTP/",
                                        "MCP-Session-Id: roots-session",
                                        "\"id\":2", "\"method\":\"ping\""};
  static const char *roots_response_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: roots-session",
      "\"id\":\"roots-1\"",
      "\"result\":{\"roots\":[{\"uri\":\"file:///tmp/cai-root\","
      "\"name\":\"cai-root\"}]}"};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
  ...
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_roots_list_request_mock", script,
          sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  ...
  config.receiver.list_roots = test_mcp_roots_list_callback;
  config.receiver.roots_list_changed = 1;
  expect_int(state, "mcp_streamable_roots_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_roots_ping",
             cai_mcp_client_ping(client, &error), CAI_OK);
  expect_int(state, "mcp_streamable_roots_count", roots.count, 1L);
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
...
static void
test_mcp_streamable_http_roots_list_callback_error(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
  ...
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_roots_list_callback_error_mock", script,
          sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  ...
  config.receiver.context = &roots;
  config.receiver.list_roots = test_mcp_roots_list_callback;
  expect_int(state, "mcp_streamable_roots_error_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_roots_error_ping",
             cai_mcp_client_ping(client, &error), CAI_ERR_CANCELLED);
  expect_int(state, "mcp_streamable_roots_error_count", roots.count, 1L);
  expect_str(state, "mcp_streamable_roots_error_message", error.message,
             "roots callback failed");
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_roots_list_callback_error_mock",
                    server.pid, &server.child_status);
}

static void
test_mcp_streamable_http_roots_list_invalid_result(test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
      "\",\"capabilities\":{},\"serverInfo\":{\"name\":\"mock-mcp\","
  ...
  memset(&server, 0, sizeof(server));
  memset(&roots, 0, sizeof(roots));
  roots.result_json = "{\"roots\":[";
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_roots_list_invalid_result_mock", script,
          sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  ...
  config.receiver.context = &roots;
  config.receiver.list_roots = test_mcp_roots_list_callback;
  expect_int(state, "mcp_streamable_roots_invalid_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_roots_invalid_ping",
             cai_mcp_client_ping(client, &error), CAI_ERR_PROTOCOL);
...
test_mcp_streamable_http_task_request_capabilities(test_state *state) {
  ...
  static const char *init_required[] = {
      ...
      "\"form\":{},\"url\":{}},\"tasks\":{\"requests\":{\"sampling\":{"
      "\"createMessage\":{}},\"elicitation\":{\"create\":{}}}}"};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: task-cap-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {"POST /v1/mcp HTTP/",
                                        "MCP-Session-Id: task-cap-session",
                                        "\"id\":2", "\"method\":\"ping\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
  ...
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_task_request_capabilities_mock", script,
          sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  ...
  config.receiver.task_sampling_create_message = 1;
  config.receiver.task_elicitation_create = 1;
  expect_int(state, "mcp_streamable_task_caps_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_task_caps_ping",
             cai_mcp_client_ping(client, &error), CAI_OK);
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state, "mcp_streamable_task_request_capabilities_mock",
                    server.pid, &server.child_status);
}

static void test_mcp_streamable_http_task_request_flags_require_callbacks(
    test_state *state) {
  static const char initialize_body[] =
      "{\"jsonrpc\":\"2.0\",\"id\":1,\"result\":{\"protocolVersion\":"
      "\"" CAI_MCP_PROTOCOL_VERSION
  ...
  static const char *init_required[] = {"POST /v1/mcp HTTP/", "\"id\":1",
                                        "\"method\":\"initialize\"",
                                        "\"capabilities\":{}"};
  static const char *init_forbidden[] = {
      "\"tasks\":{\"requests\":", "\"sampling\":{\"createMessage\":{}",
      "\"elicitation\":{\"create\":{}"};
  static const char *initialized_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: task-cap-empty-session",
      "\"method\":\"notifications/initialized\""};
  static const char *ping_required[] = {
      "POST /v1/mcp HTTP/", "MCP-Session-Id: task-cap-empty-session",
      "\"id\":2", "\"method\":\"ping\""};
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), init_forbidden,
       sizeof(init_forbidden) / sizeof(init_forbidden[0]), 200, "OK",
       "application/json", "req-init\r\nMCP-Session-Id: task-cap-empty-session",
       initialize_body},
      {"POST /v1/mcp HTTP/", initialized_required,
       sizeof(initialized_required) / sizeof(initialized_required[0]), NULL, 0U,
       200, "OK", "application/json", NULL, "{}"},
  ...
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_task_request_flags_require_callbacks_mock",
          script, sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  snprintf(url, sizeof(url), "%s/mcp", server.base_url);
  ...
  config.receiver.task_sampling_create_message = 1;
  config.receiver.task_elicitation_create = 1;
  expect_int(state, "mcp_streamable_task_flags_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
  expect_int(state, "mcp_streamable_task_flags_ping",
             cai_mcp_client_ping(client, &error), CAI_OK);
  cai_mcp_client_destroy(client);
  cai_error_cleanup(&error);
  expect_child_exit(state,
                    "mcp_streamable_task_request_flags_require_callbacks_mock",
                    server.pid, &server.child_status);
}
...
test_mcp_streamable_http_server_request_unsupported(test_state *state) {
  ...
  static const mock_http_expectation script[] = {
      {"POST /v1/mcp HTTP/", init_required,
       sizeof(init_required) / sizeof(init_required[0]), NULL, 0U, 200, "OK",
       "application/json", "req-init\r\nMCP-Session-Id: server-request-session",
       initialize_body},
      {"POST /v1/mcp HTTP/", initialized_required,
       sizeof(initialized_required) / sizeof(initialized_required[0]), NULL, 0U,
       200, "OK", "application/json", NULL, "{}"},
  ...
  cai_error_init(&error);
  if (http_mock_server_open_script(
          state, "mcp_streamable_server_request_unsupported_mock", script,
          sizeof(script) / sizeof(script[0]), &server) != 0) {
    cai_error_cleanup(&error);
    return;
  }
  ...
  config.url = url;
  config.timeout_ms = 500L;
  expect_int(state, "mcp_streamable_server_request_open",
             cai_mcp_streamable_http_client_open(&config, &client, &error),
             CAI_OK);
...
static int mock_websocket_write_frame(int fd, int fin, int opcode,
                                      const char *payload, size_t length) {
  unsigned char header[10];
  size_t header_len;

  header[0] = (unsigned char)((fin ? 0x80U : 0x00U) | (opcode & 0x0f));
  if (length < 126U) {
    header[1] = (unsigned char)length;
    header_len = 2U;
  } else if (length <= 0xffffU) {
...
static int mock_websocket_write_text_fragmented(int fd, const char *first,
                                                const char *second) {
  return mock_websocket_write_frame(fd, 0, 0x1, first, strlen(first)) == 0 &&
                 mock_websocket_write_frame(fd, 1, 0x0, second,
                                            strlen(second)) == 0
             ? 0
             : -1;
}
...
static void mock_websocket_fragmented_completed_child(int pipe_fd) {
  static const char delta[] =
      "{\"type\":\"response.output_text.delta\",\"delta\":\"fragmented ws\"}";
  static const char completed_first[] =
      "{\"type\":\"response.completed\",\"response\":{\"id\":";
  static const char completed_second[] =
      "\"resp_ws_fragmented\",\"usage\":{\"input_tokens\":2,"
      "\"input_tokens_details\":{\"cached_tokens\":1},\"output_tokens\":3,"
      "\"output_tokens_details\":{\"reasoning_tokens\":1},\"total_tokens\":5}}"
      "}";
  char request[4096];
  char message[8192];
  int server_fd;
  ...
  if (mock_websocket_write_text(client_fd, delta) != 0 ||
      mock_websocket_write_text_fragmented(client_fd, completed_first,
                                           completed_second) != 0) {
    _exit(5);
  }
  close(client_fd);
...
static void test_http_create_response_upload_setup_failure(test_state *state) {
  cai_client_config config;
  cai_client *client;
  cai_response_create_params *params;
  cai_response *response;
  ...
  cai_error_init(&error);
  lonejson_error_init(&json_error);
  CAI_LJ->spooled_init(CAI_LJ, &text);
  cai_client_config_init(&config);
  ...
  expect_int(state, "http_upload_setup_client_open",
             cai_client_open(&config, &client, &error), CAI_OK);
  expect_int(state, "http_upload_setup_params_new",
             cai_response_create_params_new(&params, &error), CAI_OK);
  expect_int(state, "http_upload_setup_params_model",
             params->set_model(params, CAI_MODEL_GPT_5_NANO, &error), CAI_OK);
  expect_int(state, "http_upload_setup_spool_append",
             text.append(&text, "large spooled setup failure input",
                         strlen("large spooled setup failure input"),
                         &json_error),
             LONEJSON_STATUS_OK);
  expect_int(state, "http_upload_setup_params_input",
             params->add_text_spooled(params, "user", &text, &error), CAI_OK);

  active_before = cai_test_response_request_upload_active();
  alloc_state.fail_after = alloc_state.allocs;
  expect_int(state, "http_upload_setup_create_response",
             cai_client_create_response(client, params, &response, &error),
             CAI_ERR_NOMEM);
  expect_str(state, "http_upload_setup_error", error.message,
             "failed to allocate URL");
  expect_int(state, "http_upload_setup_active_uploads",
             cai_test_response_request_upload_active(), (long)active_before);

  cai_response_destroy(response);
  cai_response_create_params_destroy(params);
  cai_client_close(client);
  cai_error_cleanup(&error);
  expect_int(state, "http_upload_setup_alloc_balance", alloc_state.frees,
             (long)alloc_state.allocs);
}

static void test_http_create_response_output_validation(test_state *state) {
  static const char server_error_body[] =
      "{\"error\":{\"message\":\"server rejected request\",\"type\":"
      "\"server_error\",\"code\":\"temporary\"}}";
  static const char *required[] = {"POST /v1/responses HTTP/",
                                   "\"text\":\"stale output\""};
  static const mock_http_expectation error_script[] = {
      {"POST /v1/responses HTTP/", required,
       sizeof(required) / sizeof(required[0]), NULL, 0U, 500,
       "Internal Server Error", "application/json", "req_stale",
       server_error_body}};
  http_mock_server no_request_server;
  http_mock_client error_mock;
  cai_client_config config;
  cai_client *client;
  cai_response_create_params *params;
  cai_response *stale_response;
  cai_error error;
  int no_request_opened;

  memset(&no_request_server, 0, sizeof(no_request_server));
  no_request_server.pid = -1;
  memset(&error_mock, 0, sizeof(error_mock));
  error_mock.pid = -1;
  client = NULL;
  params = NULL;
  stale_response = NULL;
  ...
  if (http_mock_server_open_no_request(state, "http_create_null_out_no_request",
                                       &no_request_server) != 0) {
    goto cleanup;
  }
  no_request_opened = 1;
  cai_client_config_init(&config);
  config.api_key = "mock-key";
  config.base_url = no_request_server.base_url;
  config.http_2_disabled = 1;
  config.timeout_ms = 500L;
  expect_int(state, "http_create_null_out_client",
             cai_client_open(&config, &client, &error), CAI_OK);
  expect_int(state, "http_create_null_out_params",
             cai_response_create_params_new(&params, &error), CAI_OK);
  expect_int(state, "http_create_null_out_model",
             params->set_model(params, CAI_MODEL_GPT_5_NANO, &error), CAI_OK);
  expect_int(state, "http_create_null_out_input",
             params->add_text(params, "user", "must not post", &error), CAI_OK);
  expect_int(state, "http_create_null_out_call",
             cai_client_create_response(client, params, NULL, &error),
             CAI_ERR_INVALID);
  expect_str(state, "http_create_null_out_error", error.message,
  ...
  if (http_mock_client_open_script(
          state, "http_create_stale_out_mock", error_script,
          sizeof(error_script) / sizeof(error_script[0]), &error_mock) != 0) {
    goto cleanup;
  }
  expect_int(state, "http_create_stale_out_params",
             cai_response_create_params_new(&params, &error_mock.error),
             CAI_OK);
  expect_int(state, "http_create_stale_out_model",
             params->set_model(params, CAI_MODEL_GPT_5_NANO, &error_mock.error),
             CAI_OK);
  expect_int(
      state, "http_create_stale_out_input",
      params->add_text(params, "user", "stale output", &error_mock.error),
      CAI_OK);
  stale_response = (cai_response *)&error_mock;
  expect_int(state, "http_create_stale_out_call",
             cai_client_create_response(error_mock.client, params,
                                        &stale_response, &error_mock.error),
             CAI_ERR_SERVER);
  if (stale_response != NULL) {
    test_fail(state, "http_create_stale_out_cleared",
              "failed request left stale response pointer");
  }
...
cleanup:
  ...
  cai_client_close(client);
  cai_error_cleanup(&error);
  if (no_request_opened) {
    expect_child_exit(state, "http_create_null_out_no_request",
                      no_request_server.pid, &no_request_server.child_status);
  }
  if (error_mock.pid > 0 || error_mock.client != NULL) {
    http_mock_client_close(state, "http_create_stale_out_mock", &error_mock);
  }
}
...
test_stream_responses_websocket_fragmented_event(test_state *state) {
  ...
  if (websocket_mock_server_open_child(
          state, "stream_ws_fragmented_mock",
          mock_websocket_fragmented_completed_child, &server) != 0) {
    goto cleanup;
  }
  server_opened = 1;
  ...
  config.http_2_disabled = 1;
  config.timeout_ms = 500L;
  expect_int(state, "stream_ws_fragmented_client",
             cai_client_open(&config, &client, &error), CAI_OK);
  expect_int(state, "stream_ws_fragmented_params",
             cai_response_create_params_new(&params, &error), CAI_OK);
  expect_int(state, "stream_ws_fragmented_model",
             params->set_model(params, CAI_MODEL_GPT_5_NANO, &error), CAI_OK);
  expect_int(state, "stream_ws_fragmented_previous",
             params->set_previous_response_id(params, "resp_ws_prev", &error),
             CAI_OK);
  expect_int(state, "stream_ws_fragmented_text",
             params->add_text(params, "user", "ws user turn", &error), CAI_OK);
  sink_callbacks.write = test_write;
  sink_callbacks.close = test_write_close;
  sink_callbacks.context = &writer;
  expect_int(state, "stream_ws_fragmented_sink",
             cai_sink_from_callbacks(&sink_callbacks, &sink, &error), CAI_OK);
  cai_stream_sinks_init(&stream_sinks);
  stream_sinks.output_text = sink;
  expect_int(state, "stream_ws_fragmented_run",
             cai_client_stream_response_websocket_test(
                 client, params, &stream_sinks, &response_id, &usage, &error),
             CAI_OK);
  expect_str(state, "stream_ws_fragmented_output", writer.buffer,
             "fragmented ws");
  expect_str(state, "stream_ws_fragmented_response_id", response_id,
             "resp_ws_fragmented");
  expect_int(state, "stream_ws_fragmented_usage_total", usage.total_tokens,
             5LL);
...
cleanup:
  ...
  cai_error_cleanup(&error);
  if (server_opened) {
    expect_child_exit(state, "stream_ws_fragmented_mock", server.pid,
                      &server.child_status);
  }
}
...
static void test_stream_upload_setup_failure(test_state *state) {
  cai_client_config config;
  cai_agent_config agent_config;
  cai_client *client;
  cai_agent *agent;
  ...
  expect_int(state, "stream_upload_setup_client_open",
             cai_client_open(&config, &client, &error), CAI_OK);
  expect_int(state, "stream_upload_setup_agent",
             cai_client_new_agent(client, &agent_config, &agent, &error),
             CAI_OK);
  expect_int(state, "stream_upload_setup_session",
             cai_agent_new_session(agent, &session, &error), CAI_OK);
  expect_int(state, "stream_upload_setup_sink",
             cai_sink_from_callbacks(&sink_callbacks, &sink, &error), CAI_OK);
  expect_int(state, "stream_upload_setup_spool_append",
             text.append(&text, "large spooled streaming setup failure input",
                         strlen("large spooled streaming setup failure input"),
                         &json_error),
             LONEJSON_STATUS_OK);
  expect_int(state, "stream_upload_setup_add",
             session->add_user_text_spooled(session, &text, &error), CAI_OK);

  active_before = cai_test_response_request_upload_active();
  alloc_state.fail_after = alloc_state.allocs;
  expect_int(state, "stream_upload_setup_run",
             cai_session_stream_text(session, sink, &error), CAI_ERR_NOMEM);
  expect_str(state, "stream_upload_setup_error", error.message,
             "failed to allocate URL");
  expect_int(state, "stream_upload_setup_active_uploads",
             cai_test_response_request_upload_active(), (long)active_before);
  ...
  cai_client_close(client);
  cai_error_cleanup(&error);
  expect_int(state, "stream_upload_setup_alloc_balance", alloc_state.frees,
             (long)alloc_state.allocs);
}
...
static const test_entry test_entries[] = {
    ...
    {"mcp_streamable_http_response_jsonrpc_invalid",
     test_mcp_streamable_http_response_jsonrpc_invalid},
    ...
    {"mcp_streamable_http_response_missing_result_error",
     test_mcp_streamable_http_response_missing_result_error},
    ...
    {"mcp_streamable_http_notification_callback_error",
     test_mcp_streamable_http_notification_callback_error},
    ...
    {"mcp_streamable_http_task_request_flags_require_callbacks",
     test_mcp_streamable_http_task_request_flags_require_callbacks},
    ...
    {"http_create_response_upload_setup_failure",
     test_http_create_response_upload_setup_failure},
    {"http_create_response_output_validation",
     test_http_create_response_output_validation},
